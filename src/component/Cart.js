...
const Cart = ({ item }) => {
  ...
  // --- to0 much rerendering of alert component here i'll solve it leater : may be with use memo
  // if(result.isSuccess){
    ...
    <div className='flex flex-row px-1 py-2 my-2  bg-gray-100/60 text-gray-700 gap-3 ' key={id}>
      ...
      <div className=' px-2  w-full relative  '>
        <div className='mb-5 pr-12'>
          {/* title */}
          <Link to={`/cart/${id}`} className='hover:underline underline-offset-4 c'> {title}</Link>
        </div>
        {/* remove icon  */}
