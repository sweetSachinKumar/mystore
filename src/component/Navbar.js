      ...
      <div className='z-30 shadow shadow-neutral-800/10 transition-shadow fixed left-0 right-0 top-0 bg-neutral-100/80 w-full'>
        <div className='  container mx-auto px-4 py-2  items-center justify-between  hidden lg:flex '>
          ...
          <Link to="/gotocart" className=' hover:text-neutral-700/80  transition active:text-neutral-950/70 text-neutral-800  text-xl px-6 py-1 rounded-md relative' >
              ...
              <IoMdCart size={30}/> 
               {data && <span className='text-neutral-100 font-bold font-mono bg-orange-600/80  px-2 rounded-xl absolute z-40 -bottom-1 right-3'>{data.allcartItem?.length}</span>}
      ...
      <div className=' container  mx-auto px-2   lg:hidden block'>
        ...
        <div className='flex justify-between items-center '>
          ...
          <div className='flex  gap-4'>
            ...
            <div onClick={() => setIsOpen(!isOpen)} className='text-2xl bg-amber-500/40 p-2 inline-block rounded-md cursor-pointer transition'>
        ...
        <nav className={` px-2 transition-all duration-300 absolute top-[3rem] ${isOpen? "left-0 translate-x-0 translate-y-0":"left-full translate-x-24 translate-y-16"} w-full  h-[300px] z-40 bg-neutral-200/80 transform   `}>
